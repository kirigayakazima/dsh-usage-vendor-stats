...
function apply(ctx) {
  ...
  // ---- health / performance aggregation ----
  // provider -> error count (from agent/request-error events)
  const errors = new Map()
  let errorTotal = 0
  // peak context: max (inputTokens + cacheReadTokens) across all assistant/message calls
  let peakContext = 0
  // sessionStats fold: summed across sessions via ctx.sessionProjections
  const perf = { llmMs: 0, toolMs: 0, ttftMs: 0, ttftSteps: 0, decodeMs: 0, decodeTokens: 0, steps: 0, turns: 0, sessionsRead: 0 }
  const perfSessions = new Set()
  ...
  function addUsage(vendorId, modelKey, time, usage, isTurnEnd) {
    ...
    // track peak context (total input = uncached input + cache read)
    const ctxTokens = input + cacheRead
    if (ctxTokens > peakContext) peakContext = ctxTokens
    ...
    for (const record of records) {
      ...
      await enqueue(sid, async () => {
        try {
          ...
          await foldSessionStats(sid, snap && snap.events)
        } catch (err) {
  ...
  // Failed model-request attempts -> error count (per provider + total)
  ctx.on('agent/request-error', (payload, next) => {
    try {
      const provider = payload && typeof payload.provider === 'string' ? payload.provider : 'unknown'
      errors.set(provider, (errors.get(provider) || 0) + 1)
      errorTotal += 1
    } catch (err) { /* count best-effort */ }
    return next()
  })

  // Fold one session's sessionStats projection into global perf (TTFT, decode, llm/tool ms)
  // Cold (persisted) sessions: fold via sessionProjections.restore() over the events we
  // already read, so no live in-memory Session is required.
  async function foldSessionStats(sid, events) {
    if (perfSessions.has(sid)) return
    const projections = ctx.get('sessionProjections')
    if (projections === undefined) return
    try {
      let v = null
      if (Array.isArray(events) && events.length > 0) {
        const restored = projections.restore({}, events, 0)
        v = restored && restored.snapshot && restored.snapshot.values && restored.snapshot.values.sessionStats
      }
      if (v === null || v === undefined) {
        const sessions = ctx.get('sessions')
        const session = sessions !== undefined ? sessions.get(sid) : undefined
        if (session !== undefined && session !== null) {
          const snap = projections.snapshot(session)
          v = snap && snap.values && snap.values.sessionStats
        }
      }
      if (v === null || v === undefined) return
      perf.llmMs += num(v.llmMs)
      perf.toolMs += num(v.toolMs)
      perf.ttftMs += num(v.ttftMs)
      perf.ttftSteps += num(v.ttftSteps)
      perf.decodeMs += num(v.decodeMs)
      perf.decodeTokens += num(v.decodeTokens)
      perf.steps += num(v.steps)
      perf.turns += num(v.turns)
      perf.sessionsRead += 1
      perfSessions.add(sid)
    } catch (err) { /* skip unreadable session stats */ }
  }
  ...
  function snapshot() {
    ...
    return {
      ...
      health: {
        errors: errorTotal,
        errorsByVendor: Array.from(errors, (p) => ({ vendorId: p[0], errors: p[1] })),
        peakContext: peakContext,
        avgTtftMs: perf.ttftSteps > 0 ? perf.ttftMs / perf.ttftSteps : null,
        ttftSteps: perf.ttftSteps,
        genTokensPerSec: perf.decodeMs > 0 ? (perf.decodeTokens / (perf.decodeMs / 1000)) : null,
        llmMs: perf.llmMs,
        toolMs: perf.toolMs,
        steps: perf.steps,
        turns: perf.turns,
        sessionsRead: perf.sessionsRead,
      },
    }
  }
